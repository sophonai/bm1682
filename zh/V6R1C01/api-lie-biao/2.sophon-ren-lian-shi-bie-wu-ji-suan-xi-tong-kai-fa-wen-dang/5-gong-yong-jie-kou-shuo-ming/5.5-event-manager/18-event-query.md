# 获取单个识别记录/sophon/v2/face/getevent（POST）

**版本**

2.0

**功能描述**

获取单个识别记录详细信息。

**调用URL**

http://192.168.150.1:5555/sophon/v2/face/getevent

调用方法: POST

**请求参数**

| 一级参数 | 类型   |   默认值   | 选择 | 描述                                                         |   举例   |
| :------- | :----- | ----------------------------------- | :--- | :----------------------------------------------------------- | --------------------------------- |
| event_id | string | | 必选 | 识别记录的唯一标识，查询单个id | 1628066406652506 |

请求示例：

```json
请求url: http://192.168.150.1:5555/sophon/v2/face/getevent
请求方法：POST
请求body:
{
  "event_id":"1628066406652506"
}
```

**返回信息：**

**请求成功**

返回event字段（[详细信息参考](13-event-detail.md)）

请求成功示例：

```json
{
    "event_id": "1628066406652506",
    "ext_data": {
        "in_timestamp": 1628066406.608026,
        "recognition_timestamp": 1628066406.608026
    },
    "frame_id": 0,
    "identity_id": "",
    "identity_name": "",
    "media_device_info": {
        "media_device_id": "",
        "media_device_ip": "",
        "media_device_name": "",
        "media_device_type": ""
    },
    "permission_id": "",
    "permission_name": "",
    "post_action": "0",
    "recognize_face_attr": {
        "age": null,
        "box": [
            169.91207885742188,
            256.9922180175781,
            487.3203430175781,
            671.7279663085938
        ],
        "expression": null,
        "feature_binary": "",
        "gender": null,
        "glasses": null,
        "hat": null,
        "mask": null,
        "point": []
    },
    "recognize_face_image_id": "001628066406.650539",
    "register_image_id": "",
    "score": 27.69,
    "spot_image_id": "001628066406.650539",
    "timestamp": 1628066406.608026,
    "user_type": "stranger"
}
```



请求失败

| 一级参数 | 二级参数 | 类型   | 描述       |
| :------- | :------- | :----- | :--------- |
| error    | code     | string    | 错误码 |
|          | message  | string | 错误描述信息   |

**错误码**

| Index | 错误码            | message     | 描述                                                         |
| ----- | ----------------- | ----------- | ------------------------------------------------------------ |
| 1     | MISSING_ARGUMENTS | <key_name>  | 缺乏必要的参数。<br /><key_name> :  缺失的参数在message字段中描述 |
| 2     | ID_NOT_FOUND      | <key_value> | ID未找到错误<br /><key_value> : 参数的值                     |

